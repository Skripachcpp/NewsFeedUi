  ...
  return new Promise((resolve, reject) => {
    ...
    client.get(url, (res) => {
      if (res.statusCode !== 200) {
        reject(new Error(`Ошибка загрузки: ${res.statusCode} ${res.statusMessage}`));
        return;
      }
...
function generateApi() {
  ...
  try {
    ...
    try {
      ...
      requestContent = requestContent.replace(/import FormData from 'form-data';\n/g, '');
      ...
      console.log(`✅ Исправлена проблема с form-data для браузерного окружения`);
    } catch (fixError) {
      console.warn(`⚠️  Не удалось применить исправления к request.ts: ${fixError.message}`);
    }
...
async function main() {
  ...
  try {
    ...
    if (useLocal) {
      console.log(`ℹ️  Используется локальный файл ${SWAGGER_FILE} (флаг --local)`);
      if (!existsSync(SWAGGER_FILE)) {
        throw new Error(`Локальный файл ${SWAGGER_FILE} не найден. Убедитесь, что файл существует или запустите без флага --local для загрузки с сервера.`);
      }
    } else {
